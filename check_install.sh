    ...
    for (const section of ['http', 'tcp']) {
        if (!original[section]) {
            ...
            for (const subsection of ['routers', 'services', 'middlewares']) {
                ...
                } else if (section === 'tcp' && subsection !== 'middlewares' && !original[section][subsection]) {
                    differences.push(`Missing ${section}.${subsection} section`);
                }
            }
        }
...
async function checkDockerCompose() {
    try {
        ...
        if (content.includes('"27017:27017"')) {
            ...
            console.error('❌ MongoDB port 27017 is not defined in docker-compose.yml');
            return false;
        }
    } catch (err) {
...
async function fixDockerCompose() {
    try {
        ...
        // Check if MongoDB port is defined
        if (content.includes('"27017:27017"')) {
            console.log('✅ MongoDB port 27017 is already defined in docker-compose.yml');
            return true;
        }
        ...
        for (const pattern of patterns) {
            const testContent = updatedContent.replace(pattern.regex, pattern.replacement);
        ...
        if (updatedContent === content) {
            console.error('❌ Could not modify docker-compose.yml - no matching patterns');
            return false;
        }
    ...
    for (const dir of agentDirs) {
        try {
            ...
            try {
                ...
                console.error(`❌ Error creating agents directory at ${dir}:`, mkdirErr.message);
            }
        }
    }
}
...
async function main() {
    ...
    if (needsRestart && traefikRunning) {
        ...
        if (!mongoDBPortExposedAfterRestart) {
            console.warn('⚠️ MongoDB port 27017 is still not exposed in Traefik after restart');
            console.warn('⚠️ You may need to recreate the Traefik container with the correct port mapping');
        }
    }
