...
function performGracefulShutdown(server) {
  ...
  server.close((err) => {
    ...
    try {
      ...
      // No specific cleanup needed with the new architecture
  ...
  try {
    ...
    setInterval(async () => {
      try {
        ...
        // Check that the proxy service is healthy
        const proxyHealth = await coreServices.proxyService.checkHealth();

        if (!proxyHealth.success || proxyHealth.status !== "healthy") {
          appLogger.warn("Proxy service is unhealthy, attempting repair");
          await coreServices.proxyService.repair();
          appLogger.info("Proxy service repair attempt completed");
        } else {
          appLogger.info("Proxy service is healthy");
        }
